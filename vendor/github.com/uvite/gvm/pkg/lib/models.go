...
type ColumnsFields struct {
	Title       string                 `json:"title"`
	DataIndex   string                 `json:"dataIndex"`
	CommonRules map[string]json.RawMessage `json:"commonRules"`
...
// A Stage defines a step in a test's timeline.
type Columns ColumnsFields

// For some reason, implementing UnmarshalText makes encoding/json treat the type as a string.
func (s *Columns) UnmarshalJSON(b []byte) error {
	var fields ColumnsFields
	if err := json.Unmarshal(b, &fields); err != nil {
		return err
	}
	*s = Columns(fields)
...
func (s Columns) MarshalJSON() ([]byte, error) {
	return json.Marshal(ColumnsFields(s))
}

func (s *Columns) UnmarshalText(b []byte) error {
	var stage Columns
