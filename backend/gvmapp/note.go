...
func (a *App) GetDirs() RespDate {
	...
	return RespDate{Code: 200, Data: ret}
}
...
func (a *App) CreateNoteFile(dirName string) RespDate {
	pwd, _ := os.Getwd()
	// 未命名.md文件路径
	defaultFile := filepath.Join(pwd, "data", "local", "test.js")
	newFileName := time.Now().Local().Format("20060102150405")
	destFilePath := filepath.Join(pwd, "data", "local", dirName, (newFileName))
	// 判断文件是否存在
...
func (a *App) SaveNote(dirName, noteName, noteTitle, noteContent string) RespDate {
	pwd, _ := os.Getwd()
	// md文件路径
	oldFilePath := filepath.Join(pwd, "data", "local", dirName, (noteName + ".md"))
	// 重命名文件路径
	newFilePath := filepath.Join(pwd, "data", "local", dirName, (noteTitle + ".md"))
...
func (a *App) ReadNoteFile(dirName, fileName string) RespDate {
	//pwd, _ := os.Getwd()
	//filePath := filepath.Join(pwd, "data", "local", dirName, (fileName))
	localData := util.GetLocalDir()
	// fmt.Println(localData)
	filePath := filepath.Join(localData, dirName, (fileName))
...
// 读取文件
func (a *App) ParseNoteFile(dirName, fileName string) RespDate {
	localData := util.GetLocalDir()
	// fmt.Println(localData)
	filePath := filepath.Join(localData, dirName, (fileName))
	data, err := ioutil.ReadFile(filePath)
	...
	gvm.Ctx = a.Ctx
	gvm.LoadFile(filePath)
	fmt.Println(filePath, err)
	v, err := gvm.ExecFunc("ui")
	fmt.Println(v, err)
	if err != nil {
		return RespDate{Code: 500, Msg: err.Error()}
	}

	ui, err := json.Marshal(v.Export())
	var tmp interface{}
	json.Unmarshal(ui, &tmp)
	fmt.Println(ui, err)
	ret := map[string]interface{}{
		"code": string(data),
		"ui":   string(ui),
...
func (a *App) RemoveNote(dirName, fileName string) RespDate {
	...
	pwd, _ := os.Getwd()
	localFilePath := filepath.Join(pwd, "data", "local", dirName, (fileName + ".md"))
	go func() {
		wg.Add(1)
		// 删除本地端文件
