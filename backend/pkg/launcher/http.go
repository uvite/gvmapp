...
func (m *Launcher) runHTTP(opts *InfluxdOpts, router *gin.Engine) error {
	...
	router.GET("/api/updatetask", func(c *gin.Context) {
		id := c.Query("id")

		pid, _ := platform.IDFromString(id)
		v := taskmodel.TaskStatusInactive
		updatedTask, err := m.KvService.UpdateTask(c, *pid, taskmodel.TaskUpdate{Status: &v})
		if err != nil {
			fmt.Println(err)
		}
		if err != nil {
		...
		OkWithDetailed(updatedTask, "获取成功", c)
	})
	...
	router.GET("/api/run", func(c *gin.Context) {
		...
		pid, _ := platform.IDFromString(id)
		_, err := m.Executor.PromisedExecute(c, *pid)
		fmt.Println(err)
		if err != nil {
			FailWithMessage("获取失败", c)
		}
		OkWithDetailed("tas", "获取成功", c)
	})
